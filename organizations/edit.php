...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!validate_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        if ($name === '' || $code === '' || $email === '') {
            ...
            $error = 'Organization category is locked after creation. Create a new organization to use a different category.';
        } else {
...
<head>
    ...
    <title>Edit Organization · ID Card Generator</title>
            ...
            <div class="content-area">
                ...
                <div class="card">
                    <div class="card-header-custom">
                        <h5><i class="fas fa-building"></i> Edit Organization</h5>
                    </div>
                    <div class="card-body-custom">
                        <div class="page-header">
                            <h4>Update Organization Details</h4>
                            <p class="text-muted">Update the organization details and replace the logo when needed.</p>
                        </div>
                        ...
                        <form method="post" enctype="multipart/form-data" novalidate>
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    <input type="text" class="form-control" name="organization_name" required value="<?= htmlspecialchars($organization['organization_name'] ?? '') ?>" placeholder="e.g. ABC International School">
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">Organization Code <span class="text-danger">*</span></label>
                                    <input type="text" class="form-control" name="organization_code" required value="<?= htmlspecialchars($organization['organization_code'] ?? '') ?>" placeholder="e.g. ABC001">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="form-text">Allowed: JPG, PNG, WEBP (Max 2MB). Leave blank to keep current logo.</div>
                                    <?php if (!empty($organization['logo'])): ?>
                                        <div class="mt-2">
                                            <img src="<?= htmlspecialchars(get_organization_logo_path($organization['logo'])) ?>" class="logo-thumb" alt="Current logo">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" class="form-control" name="phone" value="<?= htmlspecialchars($organization['phone'] ?? '') ?>" placeholder="e.g. +1234567890">
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">Email <span class="text-danger">*</span></label>
                                    <input type="email" class="form-control" name="email" required value="<?= htmlspecialchars($organization['email'] ?? '') ?>" placeholder="e.g. info@organization.com">
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">Website</label>
                                    <input type="url" class="form-control" name="website" value="<?= htmlspecialchars($organization['website'] ?? '') ?>" placeholder="e.g. https://www.organization.com">
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">Organization Type</label>
                                    <select class="form-select" name="organization_type">
                                        <option value="company" <?= (($organization['organization_type'] ?? 'company') === 'company') ? 'selected' : '' ?>>Company</option>
                                        <option value="school" <?= (($organization['organization_type'] ?? '') === 'school') ? 'selected' : '' ?>>School</option>
                                        <option value="college" <?= (($organization['organization_type'] ?? '') === 'college') ? 'selected' : '' ?>>College</option>
                                        <option value="government" <?= (($organization['organization_type'] ?? '') === 'government') ? 'selected' : '' ?>>Government</option>
                                        ...
                                        <option value="other" <?= (($organization['organization_type'] ?? '') === 'other') ? 'selected' : '' ?>>Other</option>
                                    </select>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">Organization Category</label>
                                    <select class="form-select" name="project_type" disabled aria-describedby="projectTypeLockHelp">
                                        <option value="residence" <?= (($organization['project_type'] ?? 'corporate') === 'residence') ? 'selected' : '' ?>>Residence</option>
                                        <option value="corporate" <?= (($organization['project_type'] ?? 'corporate') === 'corporate') ? 'selected' : '' ?>>Corporate</option>
                                    </select>
                                    <div class="form-text" id="projectTypeLockHelp">Category is locked after creation. Create a new organization to use a different category.</div>
                                </div>
                                <div class="col-12">
                                    <label class="form-label">Address</label>
                                    <textarea class="form-control" name="address" rows="3" placeholder="Enter full address..."><?= htmlspecialchars($organization['address'] ?? '') ?></textarea>
                                </div>
                                <div class="col-12">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" name="status" id="status" <?= ((int)($organization['status'] ?? 0) === 1) ? 'checked' : '' ?>>
