<?php
/**
 * Users Management Module - Edit User Page
 */
session_start();
...
if (!is_super_admin($currentUser) && (int)($user['organization_id'] ?? 0) !== (int)($currentUser['organization_id'] ?? 0)) {
    $_SESSION['user_error'] = 'Access Denied. You cannot edit users outside your organization.';
    header('Location: index.php');
    exit();
}

$errors = $_SESSION['user_form_errors'] ?? [];
$oldData = $_SESSION['user_form_old'] ?? [];
unset($_SESSION['user_form_errors'], $_SESSION['user_form_old']);

$fullName = $oldData['full_name'] ?? $user['full_name'];
$email = $oldData['email'] ?? $user['email'];
$mobile = $oldData['mobile'] ?? $user['mobile'];
$orgId = $oldData['organization_id'] ?? (string)($user['organization_id'] ?? '');
$roleId = $oldData['role_id'] ?? (string)($user['role_id'] ?? '');
$status = $oldData['status'] ?? (string)$user['status'];
...
<head>
    ...
    <title>Edit User · ID Card Generator</title>
...
<body>
                    ...
                    <div class="col-lg-8 col-md-10">
                        <div class="card">
                            <div class="card-header-custom">
                                <h5><i class="fas fa-user-edit"></i> Edit User: @<?= htmlspecialchars($user['username']) ?></h5>
                                <span class="badge-custom bg-light">ID: #<?= (int)$user['id'] ?></span>
                            </div>
                                ...
                                <form action="update.php" method="post" enctype="multipart/form-data" novalidate>
                                    ...
                                    <div class="avatar-upload-container">
                                        <div class="avatar-preview-box">
                                            <img src="<?= htmlspecialchars(get_user_avatar_path($user['avatar'] ?? '')) ?>" id="avatarPreview" alt="Avatar Preview">
                                        </div>
                                        <div class="text-center">
                                            ...
                                            <div class="form-text small">Leave blank to keep current image (Max 2MB)</div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="row g-3 mb-3">
                                        ...
                                        <!-- Username (Readonly) -->
                                        <div class="col-md-6">
                                            <label for="username_disabled" class="form-label">Username</label>
                                            <input type="text" id="username_disabled" class="form-control bg-light" value="<?= htmlspecialchars($user['username']) ?>" disabled>
                                            <div class="form-text">Usernames cannot be modified.</div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="row g-3 mb-4">
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <select name="organization_id" id="organization_id" class="form-select" <?= !is_super_admin($currentUser) ? 'readonly' : '' ?>>
                                                <option value="">System Default / Global</option>
                                                <?php foreach ($scopedOrgs as $org): ?>
                                                    <option value="<?= (int)$org['id'] ?>" <?= (string)$orgId === (string)$org['id'] ? 'selected' : '' ?>>
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <select name="role_id" id="role_id" class="form-select" required>
                                                <?php foreach ($scopedRoles as $r): ?>
                                                    <option value="<?= (int)$r['id'] ?>" <?= (string)$roleId === (string)$r['id'] ? 'selected' : '' ?>>
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <?php if ($isSuperTarget): ?>
                                                <select id="status_disabled" class="form-select bg-light" disabled>
                                                    <option value="1" selected>Active (Super Admin Locked)</option>
                                                </select>
                                                <input type="hidden" name="status" value="1">
                                            <?php else: ?>
                                                <select name="status" id="status" class="form-select" required>
                                                    <option value="1" <?= (string)$status === '1' ? 'selected' : '' ?>>Active (Enabled)</option>
                                                    <option value="0" <?= (string)$status === '0' ? 'selected' : '' ?>>Inactive (Disabled)</option>
                                                </select>
                                            <?php endif; ?>
                                            <?php if ($isSuperTarget): ?>
    ...
    <script>
        ...
        if ('ontouchstart' in window) {
            document.querySelectorAll('.btn, .form-control, .form-select, .avatar-upload-btn').forEach(el => {
